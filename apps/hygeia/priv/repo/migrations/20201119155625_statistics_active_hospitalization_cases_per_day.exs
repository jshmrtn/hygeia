...
defmodule Hygeia.Repo.Migrations.StatisticsActiveHospitalizationCasesPerDay do
  @moduledoc false
      ...
      CREATE MATERIALIZED VIEW statistics_active_hospitalization_cases_per_day
        ...
        AS SELECT
          tenants.uuid, date::date, COUNT(
            DISTINCT
            CASE WHEN hospitalization IS NULL THEN NULL ELSE people.uuid END
          ) AS count
          ...
          LEFT JOIN UNNEST(cases.hospitalizations) AS hospitalization
            ON (
              (hospitalization->>'start')::date <= date AND
              (
                  (hospitalization->>'end')::date >= date OR (hospitalization->>'end') IS NULL
              )
            )
