...
defmodule HygeiaWeb.RowLive.Apply do
      ...
      cond do
        ...
        true ->
          ...
          socket =
            assign(socket,
              page_title:
                "Apply #{row.uuid} - #{Type.translate(row.import.type)} / #{inserted_at} - #{gettext("Import")} - #{gettext("Inbox")}"
            )
  ...
  defp load_data(socket, row) do
    ...
    assign(socket,
      ...
      tracers: get_users(action_plan, :tracer),
      supervisors: get_users(action_plan, :supervisor),
      autotracing_enabled_by_default: Application.fetch_env!(:hygeia, :send_autotracing_enabled)
    )
  end
