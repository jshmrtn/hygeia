...
defmodule Hygeia.Repo.Migrations.StatisticsCummulativePossibleIndexCaseEndReasonsPerformance do
  @moduledoc false
    ...
    CREATE MATERIALIZED VIEW statistics_cumulative_possible_index_case_end_reasons
      ...
      AS WITH phases AS (
        SELECT
          ...
          (phase->'details'->>'type')::case_phase_possible_index_type AS count_type,
          (phase->'details'->>'end_reason')::case_phase_possible_index_end_reason AS count_end_reason
          FROM cases
          CROSS JOIN UNNEST(cases.phases) AS phase
          WHERE (
            (phase ->> 'end')::date IS NOT NULL AND
            phase -> 'details' ->> '__type__' = 'possible_index'
          )
        )
        SELECT
          tenants.uuid,
          ...
          SUM(COUNT(DISTINCT phases.person_uuid)) OVER (
            PARTITION BY type, end_reason, tenants.uuid
            ORDER BY date::date
          )::integer AS count
          ...
          LEFT JOIN phases ON (
            ...
            type = phases.count_type AND
            (
              (end_reason IS NULL AND phases.count_end_reason IS NULL) OR
              end_reason = phases.count_end_reason
            )
          )
          GROUP BY date, type, end_reason, tenants.uuid
          ORDER BY date, type, end_reason, tenants.uuid
    """)
